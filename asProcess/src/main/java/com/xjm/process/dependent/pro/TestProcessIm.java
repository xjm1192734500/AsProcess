...
public class TestProcessIm extends AbstractProcessor {
    ...
    public synchronized void init(ProcessingEnvironment processingEnv) {
        ...
        annotationProcessorContext = new AnnotationProcessorContext(processingEnv.getElementUtils(), processingEnv.getTypeUtils(), processingEnv.getMessager(), false);
    ...
    public Set<String> getSupportedAnnotationTypes() {
        ...
        ann.add("org.demo.app.serivce.TestProcessorV1");
        return ann;
    }
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        if (!roundEnv.processingOver()) {
            ...
            //处理注解
            AtomicReference<AnnotationValue> decoratedValue = new AtomicReference<>();
            AtomicReference<GemValue<TypeMirror>> typeMirrorGemValue = new AtomicReference<>();
            ...
            for (TypeElement annotation : annotations) {
                ...
                for (Element element : elementsAnnotatedWith) {
                    AnnotationMirror annotationMirror = AnnotationElUtil.checkGetAnnotationMirror(element, "org.demo.app.serivce.TestProcessorV1");
                    Map<String, AnnotationValue> annotationValueMap = AnnotationElUtil.getAnnotationValueMap(annotationMirror, (def, va) -> {
                        ...
                        if (annotationValue != null) {
                            decoratedValue.set(annotationValue);
                            typeMirrorGemValue.set(GemValue.create(annotationValue, annotationValue1, TypeMirror.class));
                        }
                    });
            ...
            FreeWork freeWorkTest = new FreeWork();
            freeWorkTest.setFileEls(new ArrayList<>());
            freeWorkTest.setMethodEls(new ArrayList<>());
            for (TypeElement decoratorElement : mapperTypes) {

                //获取筛选的方法 获取没有重写的方法 获取指定的装饰类
                TypeElement mapperElement = (TypeElement) mTypeUtils.asElement(typeMirrorGemValue.get().get());
                ...
                //导入
                freeWorkTest.setImportSets(importStr);

                //当前类
                TypeEl decoratorEl = TypeElUtils.getType(decoratorElement.asType(), elementUtils);
                //包地址
                String packageName = decoratorEl.getPackageName();
                freeWorkTest.getImportTypeNames().add(decoratorEl.getQualifiedName());
                freeWorkTest.setDecoratorClassName(decoratorEl.getName());
                freeWorkTest.setHaveDecorator(true);
                //实现类
                TypeMirror service = typeMirrorGemValue.get().getValue();
                if (service != null) {
                    Set<String> importTypes = getImportTypes(service);
                    TypeEl type = TypeElUtils.getType(service, elementUtils);
                    if (importTypes.size() > 0) {
                        freeWorkTest.getImportSets().addAll(importTypes);
                        freeWorkTest.setHaveImpl(true);
                        freeWorkTest.setImplClassName(type.getName());
                        ...
                        freeWorkTest.getImportTypeNames().add("org.springframework.beans.factory.annotation.Autowired");
                        freeWorkTest.getFileEls().add(fileEl);
                    }
                }
                ...
                for (ExecutableElement method : mappingMethods) {
                    ...
                    Iterator<? extends TypeMirror> throwTypeIterator = thrownTypes.iterator();
                    ...
                    while (varIt.hasNext()) {
                        ...
                        if (parameterType.getKind() == TypeKind.DECLARED) {
                            ...
                            for (TypeMirror typeArgument : typeArguments) {
                                if (typeArgument.getKind() == TypeKind.TYPEVAR) {
                                    mMessager.printMessage(Diagnostic.Kind.ERROR, "class " + packageName + "." + freeWorkTest.getImplClassName() + " method.filed " + typeArgument.toString() + " Can't generate mapping method for a generic type variable target.");
                                    return false;
                                }
                            }
                        }
                        ...
                        if (returnType.getKind() == TypeKind.TYPEVAR) {
                            mMessager.printMessage(Diagnostic.Kind.ERROR, "class " + packageName + "." + freeWorkTest.getImplClassName() + " method " + method.getSimpleName().toString() + " Can't generate mapping method for a generic type variable target.");
                            return false;
                        }
                        //直接获取
...
//                        methodEl.setHaveThrownType(true);
                ...
                freeWorkTest.setMethodEls(methodEls);

                //当前类添加注解
                TypeElement componentTypeElement = elementUtils.getTypeElement("org.springframework.stereotype.Component");
                freeWorkTest.setAnnotationEls(Collections.singletonList(new AnnotationEl("@Component")));
                freeWorkTest.getImportTypeNames().add("org.springframework.stereotype.Component");

                freeWorkTest.setName("TeClass");
                freeWorkTest.setAccessibility("public");
                freeWorkTest.setPackageName(packageName);

                CreateClassTemplate.createCLass(mFiler, packageName, freeWorkTest);
            }
